    ...
    public partial class Form1 : Form
    {
      ...
      //event index change
        ...
        private void exportToolStripMenuItem_Click(object sender, EventArgs e)
        {
            //make a bmp file
            SaveFileDialog dlg = new SaveFileDialog();
            dlg.Filter = "All Files (*.*)|*.*|PNG(*.png)|*.png";
            dlg.FilterIndex = 2;
            dlg.DefaultExt = "png";
            if (DialogResult.OK == dlg.ShowDialog())
            {
                Bitmap bmpExport = new Bitmap(mapX * tileWidth, mapY * tileHeigth);
                Graphics g = Graphics.FromImage(bmpExport);

                //add data structure           
                List<TextureBrush> exportTextureList = new List<TextureBrush>();
                for (int i = 0; i < m_tileMap.Count; i++)
                {
                    Bitmap drawBitMap = new Bitmap(m_tileMap[i].TileMap.PathName);                
                    TextureBrush tb = new TextureBrush(drawBitMap);
                    exportTextureList.Add(tb);
                }

                exportTextureList[1].TranslateTransform(-32, -32);
                exportTextureList[1].TranslateTransform(-32, -32);
                exportTextureList[1].ResetTransform();
                Rectangle src = new Rectangle();
                for (int x = 0; x < mapX; x++)
                {
                    for (int y = 0; y < mapY; y++)
                    {
                        ...
                        src.Y = map[x, y].Y * tileHeigth;
                        src.Size = new Size(tileWidth, tileHeigth);
                        int locationX = x * tileWidth;
                        int locationY = y * tileHeigth;
  
                        //safe check
                        if (map[x, y].TabIndex < m_tileMap.Count)
                        {                       
                            //reset texture location
                            exportTextureList[map[x, y].TabIndex].ResetTransform();

                            //translate texture
                            float dx = -src.X + locationX;
                            float dy = -src.Y + locationY;

                            //draw the bmp
                            exportTextureList[map[x, y].TabIndex].TranslateTransform(dx,dy);                       
                            g.FillRectangle(exportTextureList[map[x, y].TabIndex], locationX, locationY, tileWidth, tileHeigth);                     
                        }
                    }
                }
                ...
                for (int i = 0; i < m_tileMap.Count; i++)
                {                 
                    exportTextureList[i].Dispose();
                }

                //recycle graphic 
                g.Dispose();

                bmpExport.Save(dlg.FileName);
                //bmpExport.Save("filepath.png", System.Drawing.Imaging.ImageFormat.Png);
                bmpExport.Dispose();
            }
